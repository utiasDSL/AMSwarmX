...
namespace JPS {
  ...
  template <int Dim> class MapUtil {
    public:
      ...
      void setFree3D(const Veci<Dim> &pn)
      {
      ...
      void setFreeVoxelAndSurroundings3D(const Veci<Dim> &center, const float d)
      {
      ...
      void setFree2D(const Veci<Dim> &pn)
      {
        int total_size = dim_(0) * dim_(1);
        int index = getIndex(pn);
        if (index >= 0 && index < total_size)
        {  // check that the point is inside the map
          map_[index] = val_free;
      ...
      // set Free all the voxels that are in a 3d cube centered at center and with side/2=d
      void setFreeVoxelAndSurroundings2D(const Veci<Dim> &center, const float d)
      {
        // std::cout << "Center is" << center.transpose() << std::endl;
        int n_voxels = std::round(d / res_ + 0.5);  // convert distance to number of voxels
        for (int ix = -n_voxels; ix <= n_voxels; ix++)
        {
          for (int iy = -n_voxels; iy <= n_voxels; iy++)
          {
            Veci<Dim> voxel = center + Veci<Dim>(ix, iy);  // Int coordinates of the voxel I'm going to clear
